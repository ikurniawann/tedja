...
3. **Fase 3 — Jadwal kerja HRIS + keterlambatan** (backlog)
   - Tabel `hris.employee_schedules` (belum ada — menu Jadwal lama mengelola
     `public.staff_schedules` domain POS, terputus dari HRIS).
   - Hitung `is_late`/`late_minutes` saat clock-in dari jadwal; statistik
     absensi sungguhan (Quick Stats masih hardcoded "-"); rekap siap payroll.
...
- [ ] QA manual: buat akun role Karyawan (ESS) → absen → ajukan cuti → approve sebagai HRD.
- [ ] Fase 3: keterlambatan terhitung otomatis dari jadwal.
