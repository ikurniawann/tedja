-- Default production output type per product master (Finished Good vs Work-in-Progress).

ALTER TABLE item.products
  ADD COLUMN IF NOT EXISTS production_output_type text NOT NULL DEFAULT 'FINISHED_GOOD';

ALTER TABLE item.products
  DROP CONSTRAINT IF EXISTS products_production_output_type_check;

ALTER TABLE item.products
  ADD CONSTRAINT products_production_output_type_check
  CHECK (production_output_type = ANY (ARRAY['FINISHED_GOOD'::text, 'WIP'::text]));

COMMENT ON COLUMN item.products.production_output_type IS
  'Default production output when creating a production order for this product.';
