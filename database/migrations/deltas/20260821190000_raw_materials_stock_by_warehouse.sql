-- -----------------------------------------------------------------------------
-- v_raw_materials_stock_by_warehouse
--
-- v_raw_materials_stock menjumlahkan inventory dengan GROUP BY raw_material_id
-- saja, sehingga stok yang tampil selalu agregat SEMUA stall — switcher stall
-- di sidebar tidak pernah mengubah angkanya.
--
-- View ini menambah dimensi warehouse: satu baris per (bahan baku × stall).
-- Bahan baku yang belum punya baris inventory di stall tersebut tetap muncul
-- dengan stok 0, supaya daftar master tidak berubah jumlahnya saat ganti stall.
--
-- Kolomnya sengaja identik dengan v_raw_materials_stock (plus warehouse_id)
-- agar caller cukup menukar nama view tanpa mengubah mapping baris.
-- -----------------------------------------------------------------------------
...
SELECT rm.id,
  ...
  FROM raw_materials rm
    -- Setiap bahan baku dipasangkan dengan stall di cabangnya (bahan tanpa
    -- cabang = template global, berlaku untuk semua stall aktif).
    CROSS JOIN LATERAL (
      SELECT w.id
      FROM configuration.warehouses w
      WHERE w.is_active = true
        AND (rm.branch_id IS NULL OR w.branch_id = rm.branch_id)
    ) wh
    ...
    LEFT JOIN ( SELECT inv.raw_material_id,
      ...
      ON rm.id = i.raw_material_id AND wh.id = i.warehouse_id
