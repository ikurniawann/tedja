-- EPIC-028 Fase B1 — menu Season Pass (penerbitan pass di loket).
-- Akses: operator loket (super_admin + pos_supervisor + pos), sejajar Loket.
-- Idempoten (ON CONFLICT), reversible.

INSERT INTO iam.menus (code, menu_name, route_path, icon, menu_type, order_number, permission_context)
VALUES ('ticketing.passes', 'Season Pass',
        '/dashboard/ticketing/passes', 'ticket', 'sidebar', 6,
...
WHERE code = 'ticketing.passes';
UPDATE iam.menus child SET parent_id = parent.id
FROM iam.menus parent
WHERE child.code = 'ticketing.passes' AND parent.code = 'ticketing';

INSERT INTO iam.role_menu_permissions (role_id, menu_id, granted_actions)
SELECT r.id, m.id, COALESCE(m.permission_context->'actions', '["read"]'::jsonb)
FROM iam.roles r CROSS JOIN iam.menus m
WHERE r.code IN ('super_admin', 'pos_supervisor', 'pos')
  AND m.code = 'ticketing.passes'
