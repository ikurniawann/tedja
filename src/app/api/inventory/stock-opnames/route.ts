...
export async function GET(request: NextRequest) {
  try {
    ...
    if (params.reason) {
      conditions.push(`so.reason = $${idx++}`);
      values.push(params.reason);
    }
    if (params.search) {
      conditions.push(
        `(so.opname_number ILIKE $${idx} OR so.notes ILIKE $${idx})`
      );
    ...
    if (branchId) {
      conditions.push(`so.branch_id = $${idx++}`);
    ...
    console.error("GET stock-opnames:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await listWarehouseInventoryForOpname(validated.warehouse_id);
    if (rows.length === 0) {
      return Response.json(
        {
          success: false,
          message: "Tidak ada bahan baku aktif di cabang gudang ini",
        },
        { status: 400 }
      );
    }
    ...
    const detail = await fetchStockOpnameDetail(header.id);
    return Response.json({
      success: true,
      data: detail,
    ...
    if (error instanceof ApiError) return error.toResponse();
    console.error("POST stock-opnames:", error);
    if (error instanceof z.ZodError) {
      return Response.json(
        { success: false, message: "Validasi gagal", errors: error.flatten().fieldErrors },
        { status: 400 }
