...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    console.error("GET stock-opname detail:", error);
    return Response.json(
      { success: false, message: "Gagal mengambil detail stock opname" },
      { status: 500 }
    );
  }
}
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    const refreshed = await fetchStockOpnameDetail(id);
    if (!refreshed) {
      return Response.json(
        { success: false, message: "Stock opname tidak ditemukan" },
        { status: 404 }
    ...
    return Response.json({
      success: true,
      data: updated,
      message: "Perubahan stock opname disimpan",
    });
  } catch (error: unknown) {
    if (error instanceof ApiError) return error.toResponse();
    console.error("PATCH stock-opname:", error);
