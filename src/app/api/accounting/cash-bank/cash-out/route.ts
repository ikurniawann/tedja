...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("GET /api/accounting/cash-bank/cash-out", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: true,
          data,
          message: "Cash Out berhasil dicatat & diposting",
        },
        { status: 201 }
      );
    } catch (e) {
    ...
    console.error("POST /api/accounting/cash-bank/cash-out", error);
    return NextResponse.json(
      { success: false, message: errMsg(error) },
      { status: 500 }
    );
