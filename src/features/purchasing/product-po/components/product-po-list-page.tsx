...
export function ProductPOListPage() {
  ...
  const listQuery = useProductPurchaseOrderList({
    page,
    limit,
  ...
  useEffect(() => {
    if (listQuery.isError) toast.error("Failed to load purchase orders");
  }, [listQuery.isError]);
  ...
  return (
    <div className="space-y-6">
      <PurchasingPageHeader
        ...
        description={`Manage product purchase orders — ${total} total`}
        actions={
          <Link href={PRODUCT_ROUTES.purchasingPoInsert}>
      ...
      <PurchasingListSection
        ...
        description="Track product purchase orders by number, vendor, status, and total value."
        toolbar={
                ...
                <tbody className="divide-y divide-gray-100">
                  {pos.map((po) => (
                    <tr
                      ...
                      onClick={() => router.push(PRODUCT_ROUTES.purchasingPoDetail(po.id))}
                    >
                      ...
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <Link href={PRODUCT_ROUTES.purchasingPoDetail(po.id)}>
