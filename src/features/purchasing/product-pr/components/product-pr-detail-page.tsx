...
export function ProductPRDetailPage({ params }: ProductPRDetailPageProps) {
  ...
  const backHref = fromApproval ? PRODUCT_ROUTES.approvalPr : PRODUCT_ROUTES.purchasingPr;
  const backLabel = fromApproval ? "Back to Approvals" : "Back";
  const editHref = fromApproval
    ? appendNavFrom(PRODUCT_ROUTES.purchasingPrEdit(id), NAV_FROM_APPROVAL_PR)
    : PRODUCT_ROUTES.purchasingPrEdit(id);
  const { data: pr, isLoading, isError, error } = useProductPurchaseRequest(id);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 border-b border-gray-200/70 pb-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          ...
          {canCreatePO && (
            <Link href={`${PRODUCT_ROUTES.purchasingPoInsert}?pr_id=${id}`}>
          ...
          {pr.status === "converted" && pr.converted_po_id && (
            <Link href={PRODUCT_ROUTES.purchasingPoDetail(pr.converted_po_id)}>
              <Button className="purchasing-main-button w-full sm:w-auto">
                <FileText className="mr-2 h-4 w-4" />
                View Purchase Order
              </Button>
                ...
                <div className="overflow-x-auto p-4">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {pr.items.map((item: ProductPRDetailItem, index: number) => (
                        <tr key={item.id} className="hover:bg-gray-50/80">
                          <td className="px-4 py-3 text-gray-500">{index + 1}</td>
                          <td className="px-4 py-3">
                            <div className="font-medium text-gray-900">
                              {item.product?.nama || item.description || "-"}
                            </div>
                            {item.product?.kode && (
              ...
              {pr.status === "pending_head" && (
                <div className="rounded-xl border border-blue-200/80 bg-blue-50/60 p-4 text-sm text-blue-900">
                  ...
                  <p className="mt-1 text-blue-800/90">
                    This request is waiting for item and quantity approval.
                  </p>
                </div>
              )}
            </CardContent>
